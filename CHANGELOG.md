...
- **Conservative feature flags** — `supports_over_clause`, `supports_select_intersection`, `supports_select_difference`, `supports_partial_indexes`, and `supports_expression_indexes` are now `False`. Django will no longer generate SQL for these features that the cursor's regex SQL rewriter cannot reliably translate. `supports_select_union` stays `True` because Wagtail's site-history admin relies on it and the live wagtail test suite is integration coverage. Re-enable each remaining flag only after backend integration coverage lands.
